...
namespace RCS.TM
{
    public class TMApplicationService : RCSAppService, ITMApplicationService
    {
        ...
        [Authorize(RCSPermissions.InTask.Create)]
        public async Task CreateTask(CreateTaskDto input)
        {
            try
            {
                _logger.LogInformation($"通过前端提供必要参数创建任务，进入{nameof(CreateTask)}");
                _logger.LogInformation($"{nameof(CreateTask)}方法传入的参数为：任务类型：{input.TaskType},任务流程类型为：{input.FlowType}起点：{input.FromAddress},途经点：{input.MiddleAddress},终点：{input.ToAddress},取的数量：{input.FetchCount},放的数量{input.PutCount},取的类型{input.FetchType},放的类型{input.PutType},取的机器类型{input.FetchMachineType},放的机器类型{input.PutMachineType}");
