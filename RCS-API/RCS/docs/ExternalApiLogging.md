...
- **位置**: `src/RCS.Domain/LogManage/APILogs/ThirdPartyCallLog.cs`
- **功能**: 存储外部 API 调用日志
- **字段**:
  - `SysName`: 系统名称/客户端名称
  - `BusinessId`: 业务标识（可选）
...
- **位置**: `src/RCS.HttpApi/Integrations/Http/Handlers/ThirdPartyLogHandler.cs`
- **功能**: HTTP 消息处理器，自动记录所有经过的请求
- **使用方式**: 在模块中注册为 HttpClient 的消息处理器

### 3. ExternalApiLogService
- **位置**: `src/RCS.Domain/LogManage/Services/ExternalApiLogService.cs`
...
### 4. HttpClient 扩展方法
- **位置**: `src/RCS.HttpApi/Integrations/Http/Extensions/HttpClientLoggingExtensions.cs`
...
实体已在 `RCSDbContext` 中注册，并创建了数据表 `tb_ThirdPartyCallLog`。

### 服务注册

在 `RCSHttpApiHostModule` 中已自动注册：
