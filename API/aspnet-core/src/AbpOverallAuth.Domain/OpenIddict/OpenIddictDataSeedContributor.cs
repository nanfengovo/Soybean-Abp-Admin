...
namespace AbpOverallAuth.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("AbpOverallAuth") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "AbpOverallAuth", DisplayName = "AbpOverallAuth API", Resources = { "AbpOverallAuth" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        {
            var swaggerRootUrl = configurationSection["AbpOverallAuth_Swagger:RootUrl"]?.TrimEnd('/');
            ...
            // 新增支持密码模式的客户端（修正后）
            await CreateApplicationAsync(
                name: "AbpOverallAuth_Password", // 新的client_id
                ...
                permissions: new List<string>
                {
                    ...
                    OpenIddictConstants.Permissions.Scopes.Profile,
                    OpenIddictConstants.Permissions.Scopes.Roles,
                    "AbpOverallAuth" // 项目对应的scope（必须和你的项目名称一致）
                },
